...
export function WorkItemCard({ item, onOpen, dragHandleProps, isDragging }: Props) {
  return (
    <div
      className={`rounded-md border bg-white p-3 shadow-sm hover:border-primary/40 transition-colors cursor-pointer ${
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <span className="text-xs text-gray-400 font-mono">{item.item_key}</span>
      </div>

      <p className="text-sm text-gray-900 mt-2 line-clamp-3">{item.title}</p>
